...
- **Re-blast cycle (putaran) — `blast_recipients.cycle`.** Attempt tiap invoice dihitung PER-CYCLE, bukan monotonik seumur hidup. Cycle 1 = putaran normal (Attempt 1-2-3 pertama). Untuk re-blast invoice yang sudah selesai 3x: **Generate Kode & Link → centang "Mulai ulang Attempt 1"** (`reset=1`) membuka **cycle baru** (cycle+1, Attempt 1 lagi) + reset thread ke `after_blast`. Semua query "progres" (generate `invoiceCycleState`, retry `collectInvoiceRetries`/`invoiceStillNeedsRetry`/`phoneHasPendingInvoice`, report `queryUnresponsive`) memfilter **cycle terkini** = `r.cycle = (SELECT MAX(cycle) …)`. Data lama semua `cycle=1` → filter no-op (identik dgn sebelum fitur). `backfillRecipientAttempts` me-rank attempt PER (phone,invoice,**cycle**). Zopoz punya kolom `cycle` juga (selalu 1, tak ada fitur reset). Catatan: reset menghormati `keep` di `ensureThreadAfterBlast` (thread open/in_progress/on_going/scheduled/konfirmasi_web TIDAK di-reset).
